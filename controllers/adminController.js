...
const couponManagement = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1; // Get the requested page from query parameters or default to page 1
        const limit = 10; // Number of coupons per page
        const skip = (page - 1) * limit; // Calculate the number of coupons to skip

        const currentDate = new Date();
        const totalCoupons = await Coupon.countDocuments(); // Get the total count of coupons
