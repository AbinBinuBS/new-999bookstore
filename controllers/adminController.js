...
const adminDashboard = async (req,res,next) => {
  try {
    ...
    let arrayforOrder = [];
    let arrayforStatus = [];
    const monthlyUserCounts = await dashboardHelper.getMonthlyUserCount();
    monthlyUserCounts.forEach((monthData) => {
      const { month, count } = monthData;
      arrayforUser.push(count);
    });
    const monthlyOrderCounts = await dashboardHelper.getMonthlyOrderDetails();
    monthlyOrderCounts.forEach((monthData) => {
      const { month, count } = monthData;
      arrayforOrder.push(count);
    });
    const statuspersantage = await dashboardHelper.getOrderStatusPercentages();
    ...
    const revenue = await dashboardHelper.calculateDeliveredRevenue();
    const monthlyrevenue = await dashboardHelper.getCurrentMonthRevenue();
...
const reportDetails = async (req,res,next) => {
  try {
...
const activeList = async (req,res,next) => {
  try {
...
const uactiveList = async (req,res,next) => {
  try {
...
const categoryBlock = async (req,res,next) => {
  try {
...
const viewsorders = async (req,res,next) => {
  try {
    id = req.query.id;
    const orderData = await Order.findById(id).populate("items.productId");
...
// ============================Banner===============================
const banneranagement = async (req,res,next) => {
  try {
    const currentDate = new Date();
